    ...
    return (
        <>
            ...
            <div className="space-y-6 p-5">
                ...
                {/* Department Holdings Section */}
                {departmentHoldings && (
                    <div className="grid gap-4 sm:grid-cols-3">
                        <StatCard
                            label="Current Holding"
                            value={departmentHoldings.currentHolding}
                            icon={Package}
                            trend={{ value: 0, label: 'items currently available' }}
                        />
                        <StatCard
                            label="Collection (This Month)"
                            value={departmentHoldings.collectionThisMonth}
                            icon={ArrowUpRight}
                            variant="success"
                            trend={{ value: 0, label: 'items received' }}
                        />
                        <StatCard
                            label="Consumption (This Month)"
                            value={departmentHoldings.consumptionThisMonth}
                            icon={ArrowDownRight}
                            variant="danger"
                            trend={{ value: 0, label: 'items issued/used' }}
                        />
                    </div>
                )}
                ...
                {/* Empty State for Users with No Dashboard Access */}
                {!showGlobalStats && !departmentHoldings && (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[#E1E3E5] bg-gray-50/50 p-12 text-center">
                        ...
                        <p className="mt-2 max-w-md text-sm text-gray-500">
                            Your account is not currently assigned to a department, and you do not have global dashboard viewing permissions. Please contact your administrator if you believe this is an error.
                        </p>
                    </div>
                )}
            </div>
