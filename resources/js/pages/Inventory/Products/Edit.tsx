...
export default function EditProduct({ categories, product, units }: Props) {
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-8 py-8 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8">
                <PageHeader 
                    title="Edit Product" 
                    description={`Update the details for ${product.name}. Global changes will reflect across all inventory modules.`}
                />
                ...
                <form onSubmit={submit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-6">
                        <Card className="border-border/50 shadow-sm">
                            ...
                            <CardContent className="p-6 flex flex-col gap-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <Label htmlFor="barcode" className="text-sm font-semibold text-info">Barcode</Label>
                                        <Input 
                                            id="barcode"
                                            type="text" 
                                            value={data.barcode} 
                                            onChange={e => setData('barcode', e.target.value)} 
                                            className="h-11 border-info/30"
                                        />
                        ...
                        <Card className="border-border/50 shadow-sm">
                            <CardHeader className="bg-surface-header/50 border-b border-border py-4 px-6">
                                <CardTitle className="text-sm font-bold uppercase tracking-wider text-text-secondary">Inventory Control</CardTitle>
                            </CardHeader>
                            <CardContent className="p-6 flex flex-col gap-6">
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                    ...
                                    <div className="flex flex-col gap-2 md:col-span-1">
                                        <Label htmlFor="reorder_level" className="text-sm font-semibold text-warning">Low Level</Label>
                                        <Input 
                                            id="reorder_level"
                                            type="number" 
                                            value={data.reorder_level} 
                                            onChange={e => setData('reorder_level', parseInt(e.target.value))} 
                                            className="h-11 border-warning/30"
                                        />
                                        <InputError message={errors.reorder_level} />
                                    </div>
                                    <div className="flex flex-col gap-2 md:col-span-1">
                                        <Label htmlFor="reorder_quantity" className="text-sm font-semibold">Order Qty</Label>
                                        <Input 
                                            id="reorder_quantity"
                                            type="number" 
                                ...
                                <div className="flex flex-col sm:flex-row gap-8 p-6 bg-surface-header/30 rounded-xl border border-border/50">
                                    <label className="flex items-center gap-3 text-sm cursor-pointer group">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="font-bold text-text-primary italic">Expirable</span>
                                            <span className="text-[10px] text-text-muted">Batch expiry required</span>
                                        </div>
                                    </label>
                                    <label className="flex items-center gap-3 text-sm cursor-pointer group">
                                        <Checkbox 
                                        ...
                                        />
                                        <div className="flex flex-col">
                                            <span className="font-bold text-info italic">Prescription Required</span>
                                            <span className="text-[10px] text-text-muted">Clinical control required</span>
                                        </div>
                                    </label>
                                </div>
                            </CardContent>
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <Link href={`/inventory/products/${product.id}`} className="w-full">
                                <Button variant="outline" type="button" className="w-full h-12 border-border/50">
                                    Discard Changes
                                </Button>
                            </Link>
                        </div>
                        ...
                        <div className="p-4 bg-muted/30 rounded-xl border border-border/50 text-[10px] text-text-muted leading-relaxed italic">
                            Tip: SKU and Category changes may affect historical reporting and supply chain grouping.
                        </div>
                    </div>
                </form>
            </div>
